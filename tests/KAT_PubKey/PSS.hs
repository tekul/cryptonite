...
doSignTest key (i, vector) = testCase (show i) (Right (signature vector) @=? actual)
    where actual = PSS.signWithSalt (salt vector) Nothing pssParams key (message vector)

doVerifyTest key (i, vector) = testCase (show i) (True @=? actual)
    where actual = PSS.verify pssParams (private_pub key) (message vector) (signature vector)

pssParams = PSS.defaultPSSParamsSHA1 :: PSS.PSSParams SHA1 ByteString ByteString
